...
void add_new_lexical_edge1(struct lattice *lc, struct edge *e, int cfrom, int cto, char *orth)
{
	struct lattice_edge *lat_e = slab_alloc(sizeof(*lat_e));
	bzero(lat_e, sizeof(*lat_e));
	assert(e->from <= lc->nvertices);
	...
	lat_e->from = lc->vertices[e->from];
	lat_e->to = lc->vertices[e->to];
	lat_e->token = degenerate_token(cfrom, cto, orth); //*tokens;
	lat_e->edge = e;
	add_lattice_edge(lc, lat_e);
}
...
struct lattice *lexical_lookup_into_chart(struct lattice *token_chart)
{
	...
	void edge_handler(struct edge * le, struct token * *tokens)
	{
		...
		add_new_lexical_edge1(lc, le, (*tokens)->cfrom, (*tokens)->cto, (*tokens)->string);
	}
...
struct profiler *start_lrule_profiler(struct rule *r)
{
	...
	if (!profs)
		profs = calloc(sizeof(struct profiler *), nrules);
...
int lexical_parse(struct lattice *lc, struct lattice_edge *le, struct edge **epsila, char *used)
{
	...
	for (i = 0; i < nrules; i++)
	{
		...
		int fired = 0;
		if (g_profiling)
			prof = start_lrule_profiler(rules[i]);
		if (filter(epsila[i], le->edge))
		{
			...
			if (res == 1 && e != NULL)
			{
				...
				add_new_lexical_edge1(lc, e, le->token->cfrom, le->token->cto, le->token->string);
				fired = 1;
			}
			else if (trace > 1)
			{
...
void print_lexical_chart(struct lattice *tc)
{
	...
	for (i = 0; i < tc->nedges; i++)
	{
		...
		printf("lexical edge #%d le %p vtx [%d-%d] lexeme '%s' letype '%s'\n",
			   t->id, e, e->from->id, e->to->id, t->lex->word, t->lex->lextype->name);
	}
}
...
int reduce_lexical_lattice(struct lattice *lat, struct lattice *toklat)
{
	...
	if (g_profiling)
		start_and_alloc_profiler(&redprof, "chart reduction", lexical_parsing_profiler, NULL);
	...
	if (g_profiling)
		stop_profiler(redprof, lat->nedges);
